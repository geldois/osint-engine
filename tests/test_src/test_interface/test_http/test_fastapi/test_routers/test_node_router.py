...
class TestGetNodeHistoryAuthentication:
    @pytest.mark.asyncio
    async def test_missing_token_returns_401(
        self, fastapi_app_client: AsyncClient
    ) -> None:
        response = await fastapi_app_client.get(f"/nodes/{uuid4()}/history")

        assert response.status_code == 401


class TestGetNodeHistoryReadAccess:
    @pytest.mark.asyncio
