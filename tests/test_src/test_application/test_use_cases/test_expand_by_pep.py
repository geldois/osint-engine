...
from tests.fakes.fetchers import FakePEPFetcher
...
class TestExpandByPEPOrchestration:
    @pytest.mark.asyncio
    async def test_returns_the_revision_the_repository_stored(
        ...
        pep_fetcher = make_fake_pep_fetcher(revision=revision)

        use_case = ExpandByPEP(
    ...
    async def test_returns_none_and_skips_persistence_when_no_records_are_found(
        ...
        mem_storage = make_mem_storage(external_credentials=[credential])
        mem_uow = make_mem_uow(mem_storage=mem_storage)
        pep_fetcher = FakePEPFetcher(revision=None)
...
class _CountingPEPFetcher(FakePEPFetcher):
...
class TestExpandByPEPReuseLock:
    @pytest.mark.asyncio
    async def test_raises_already_fetched_without_calling_the_fetcher_again(
        ...
        mem_uow = make_mem_uow(mem_storage=mem_storage)
        pep_fetcher = _CountingPEPFetcher(revision=None)
        ...
        assert pep_fetcher.call_count == 0
    ...
    async def test_force_true_bypasses_the_lock_and_calls_the_fetcher(
        ...
        mem_uow = make_mem_uow(mem_storage=mem_storage)
        pep_fetcher = _CountingPEPFetcher(
            revision=make_entity_revision(entity=make_graph())
        )
        ...
        assert result is not None
        assert pep_fetcher.call_count == 1
