...
class _CountingCNEPFetcher(FakeCNEPFetcher):
    ...
    async def fetch(
        ...
        return await super().fetch(
            cpf_or_cnpj=cpf_or_cnpj, cnep_id=cnep_id, credential=credential
        )


class TestExpandByCNEPReuseLock:
    @pytest.mark.asyncio
    async def test_raises_already_fetched_without_calling_the_fetcher_again(
        ...
        mem_uow = make_mem_uow(mem_storage=mem_storage)
        cnep_fetcher = _CountingCNEPFetcher(revision=None)
        ...
        assert exception.value.provider == "cnep"
        assert cnep_fetcher.call_count == 0
    ...
    async def test_force_true_bypasses_the_lock_and_calls_the_fetcher(
        ...
        mem_uow = make_mem_uow(mem_storage=mem_storage)
        cnep_fetcher = _CountingCNEPFetcher(
            revision=make_entity_revision(entity=make_graph())
        )
        ...
        assert result is not None
        assert cnep_fetcher.call_count == 1
