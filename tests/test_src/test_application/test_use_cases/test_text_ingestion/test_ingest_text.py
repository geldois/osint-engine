...
_VALID_CPF_TEXT = "Contato: CPF 111.444.777-35"
...
def _pattern_set_repository() -> MemPatternSetRepository:
    return MemPatternSetRepository(pattern_sets=(BRAZILIAN_DOCUMENT_PATTERNS,))
...
class TestIngestTextCompanyAndAddressStubs:
    ...
    async def test_creates_a_stub_company_and_links_it_to_the_text_source(
        ...
        use_case = IngestText(
            ...
            pattern_set_id=BRAZILIAN_DOCUMENT_PATTERNS.id,
...
class TestIngestTextNewStub:
    ...
    async def test_creates_a_stub_person_and_links_it_to_the_text_source(
        ...
        assert persons[0].cpf == "111.444.777-35"
...
class TestIngestTextUnknownPatternSet:
    @pytest.mark.asyncio
    async def test_raises_before_extracting_anything(
        ...
        use_case = IngestText(
            uow_factory=make_mem_uow_factory(),
            pattern_set_repository=_pattern_set_repository(),
            pattern_set_id=PatternSetID("does_not_exist"),
            text=_VALID_CPF_TEXT,
        )
