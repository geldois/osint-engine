...
if TYPE_CHECKING:
    ...
    from tests.test_src.test_infrastructure.test_providers.test_portal_transparencia.test_endpoints.conftest import (  # noqa: E501
        MakePortalTransparenciaCEPIMFetcher,
    )
...
_CEPIM_RESPONSE_DATA = [_CEPIM_RECORD_DATA]


class TestPortalTransparenciaCEPIMFetcherOnHTTPStatusError:
    @pytest.mark.asyncio
    @pytest.mark.parametrize("status_code", [400, 404, 429, 500, 503])
    async def test_wraps_error_preserving_status_code(
        self,
        make_portal_transparencia_cepim_fetcher: MakePortalTransparenciaCEPIMFetcher,
        ...
        fetcher = make_portal_transparencia_cepim_fetcher(handler=handler)
...
class TestPortalTransparenciaCEPIMFetcherOnCredentialRejection:
    @pytest.mark.asyncio
...
class TestPortalTransparenciaCEPIMFetcherOnNetworkFailure:
    @pytest.mark.asyncio
...
class TestPortalTransparenciaCEPIMFetcherOnMalformedJSON:
    @pytest.mark.asyncio
...
class TestPortalTransparenciaCEPIMFetcherOnEmptyResult:
    @pytest.mark.asyncio
...
class TestPortalTransparenciaCEPIMFetcherOnSuccess:
    @pytest.mark.asyncio
    async def test_returns_a_graph_revision_stamped_at_the_fetch_boundary(
        ...
        def handler(request: Request) -> Response:  # noqa: ARG001
            return Response(200, json=_CEPIM_RESPONSE_DATA)
    ...
    @pytest.mark.asyncio
    async def test_filters_by_cnpj_sancionado_when_cepim_id_is_absent(
        self,
        make_portal_transparencia_cepim_fetcher: MakePortalTransparenciaCEPIMFetcher,
        portal_transparencia_credential: ExternalCredential,
    ) -> None:
        ...
        assert "cnpjSancionado=33754482000124" in captured["path"]
        assert "pagina=1" in captured["path"]

    @pytest.mark.asyncio
    async def test_requests_the_single_record_by_id_when_cepim_id_is_provided(
        self,
        make_portal_transparencia_cepim_fetcher: MakePortalTransparenciaCEPIMFetcher,
        portal_transparencia_credential: ExternalCredential,
    ) -> None:
        ...
        assert captured["path"].endswith("cepim/42")
