...
from osint_engine.infrastructure.providers.payload import (
    Payload,
...
class TestPayloadRequire:
    def test_raises_when_key_is_missing(self) -> None:
        payload = Payload(provider="test", data={})
    ...
    def test_raises_when_field_type_does_not_match_expected_type(self) -> None:
        payload = Payload(provider="test", data={"field": 1})

        with pytest.raises(UnexpectedFieldTypeError) as exception:
            payload.require(key="field", expected_type=str)

        assert exception.value.provider == "test"
    ...
    def test_accepts_union_expected_type(self) -> None:
        payload = Payload(provider="test", data={"field": 1.5})
    ...
    def test_rejects_value_outside_union_expected_type(self) -> None:
        payload = Payload(provider="test", data={"field": "not a number"})
    ...
    def test_cast_to_is_not_applied_when_type_check_fails(self) -> None:
        payload = Payload(provider="test", data={"field": "wrong"})
...
class TestPayloadOptional:
    ...
    def test_returns_none_when_value_is_null(self) -> None:
        payload = Payload(provider="test", data={"field": None})
...
class TestPayloadScope:
    def test_scoped_payload_reads_from_narrowed_data(self) -> None:
        payload = Payload(provider="test", data={"outer": {"inner": "value"}})
    ...
    def test_scoped_payload_preserves_provider_for_error_reporting(self) -> None:
        payload = Payload(provider="brasilapi", data={})
    ...
    def test_scope_returns_a_new_independent_instance(self) -> None:
        payload = Payload(provider="test", data={"field": "outer"})
