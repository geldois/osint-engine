...
def build_get_entity_record_catalog_handler(
    *, container: Container
) -> Callable[[dict[str, object]], Awaitable[list[EntityRecordSchema]]]:
    jwt_guard = build_jwt_guard(container=container)

    async def get_entity_record_catalog(
        payload: dict[str, object] = Depends(jwt_guard),  # noqa: B008
    ) -> list[EntityRecordSchema]:
        del payload

        use_case = container.use_cases.list_entity_record_catalog()

        records = await use_case.execute()

        return [entity_record_to_schema(record) for record in records]

    return get_entity_record_catalog
